...
public void waitForElementToBeClickable(By by, int timeSecs, String description){
	...
	try {
		...
		$(by).shouldBe(visible, Duration.ofSeconds(20));
	...
	public void waitForElementToPresent(By by, int timeSecs, String description){
		...
		try {
			...
			*/
			$(by).shouldBe( visible, Duration.ofSeconds(10)).should(appear);
			report.updateTestLog(description, "The element is present", Status.PASS);
		}catch(Exception e){
			report.updateTestLog(description, "The element is not present", Status.FAIL);
	...
	public void clickButtonIDE(SelenideElement buttonElementName, String description){
		...
		try {
			if(isElementPresent(buttonElementName)){
				buttonElementName.should(visible).click();
				...
				throw  new UnExpectedException("Button is not able to click");
			}
	...
	*/
	...
	public void pressDownAndEnter(SelenideElement ele, String description) {
		try{
			synchronized (ele){

				ele.should(visible).wait(3000);
			}
			ele.should(visible).press(Keys.ARROW_DOWN).press(Keys.ENTER);
			report.updateTestLog("Press the enter key", description, Status.PASS);
		}catch (Exception e){
			report.updateTestLog("Press the enter key", description, Status.FAIL);
		}
	...
	public void pressUpAndEnter(SelenideElement ele, String description) {
		try{
			synchronized (ele){

				ele.should(visible).wait(3000);
			}
			ele.should(visible).press(Keys.ARROW_UP).press(Keys.ENTER);
			report.updateTestLog("Press the enter key", description, Status.PASS);
		}catch (Exception e){
			report.updateTestLog("Press the enter key", description, Status.FAIL);
		}
	...
	public void sizeEquals(ElementsCollection element, int size) {
		element.shouldHave(CollectionCondition.size(size));
		report.updateTestLog("Validate the size of the element should be equal to "+size, "Working as expected", Status.PASS);
	}

	public void sizeGreater(ElementsCollection element, int size) {
		element.shouldHave(CollectionCondition.sizeGreaterThan(size));
		report.updateTestLog("Validate the size of the element should be greater than "+size, "Working as expected", Status.PASS);
	}

	public void sizeLesser(ElementsCollection element, int size) {
		element.shouldHave(CollectionCondition.sizeLessThan(size));
		report.updateTestLog("Validate the size of the element should be lesser then "+size, "Working as expected", Status.PASS);
	}
}